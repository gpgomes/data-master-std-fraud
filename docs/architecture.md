...
| Região e orçamento AWS (issue #17) | us-east-1 e teto de US$ 50/mês para a conta; MWAA, MSK, EMR e NAT são temporários (sobe → testa → derruba, atrás de flags `enable_*`); existe um único ambiente, `dev` | O orçamento não comporta stack de compute ligada: a stack completa custa cerca de US$ 0,91/h (MWAA `micro` + MSK + EMR + NAT), ou seja, ~US$ 22 por dia esquecida. Só S3, IAM, Glue, Athena e o Budget ficam ligados (~US$ 7/mês). O AWS Budgets só alerta, não bloqueia — a defesa real é o teardown (ver `docs/runbook.md`, "Infraestrutura AWS") |
| Compute e serving na AWS (planejado, issue #17) | MSK provisionado (2×`kafka.t3.small`), EMR Serverless para batch e EMR em EC2 só para o streaming, Athena + Glue como serving (sem RDS/Redshift), S3 com SSE-S3 | MSK Serverless cobra US$ 0,75/h fixos enquanto existir (~US$ 560/mês), contra ~US$ 0,09/h do provisionado; EMR Serverless não cobra ocioso e um run batch de 10 min custa ~US$ 0,08; Athena varre Parquet por ~US$ 5/TB (centavos neste volume) e Redshift Serverless tem piso de ~US$ 3/h ativo; KMS por bucket custa US$ 1/mês por chave — o módulo `s3` aceita `kms_key_arn` para ligar quando o orçamento permitir |

## CI/CD

Job `lint` (ruff + mypy), job `test` (`pytest tests/unit/`, gate de cobertura ≥70%) e job `terraform` (`make tf-check`: fmt, validate, tflint, checkov) rodam em paralelo a cada push/PR para `main`, via `.github/workflows/ci.yml`. O job `terraform-plan` (plan somente-leitura via OIDC) só liga depois do bootstrap da conta AWS. Reflete só a camada de qualidade de código da futura Fase 6 do roadmap (`CaseFinancialDataLakeHouse.md`, item 6.1) — o deploy automatizado para AWS ainda não existe.
